	...
	public function __construct()
	{
		...
		$this->statements['trinity'] = array(
			'TopArenaTeams'	=>	"SELECT 
									...
									`type`
								FROM `arena_team`
								INNER JOIN `arena_team_stats` on `arena_team_stats`.arenaTeamId = `arena_team`.arenaTeamId
								WHERE `type` = ? ORDER BY rating DESC LIMIT ?;",
			'TeamMembers'	=> 	"SELECT 
									`arena_team_member`.`arenaTeamId` AS arenateamid, 
									`arena_team_member`.`guid` AS guid, 
									`character_arena_stats`.`personal_rating` AS rating,
								...
								LEFT JOIN `character_arena_stats` ON `character_arena_stats`.`guid` = `arena_team_member`.`guid`
								WHERE `arena_team_member`.`arenateamid` = ?
								AND `character_arena_stats`.`slot` = 0
								ORDER BY guid ASC;",
